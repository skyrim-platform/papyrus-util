...
export const AddPackageOverride = (targetActor: Actor | null | undefined, targetPackage: Package | null | undefined, priority: number = 30, flags: number = 0): void => sn.AddPackageOverride(targetActor,  targetPackage,  priority,  flags)

// Remove a previously added package override.
export const RemovePackageOverride = (targetActor: Actor | null | undefined, targetPackage: Package | null | undefined): boolean => sn.RemovePackageOverride(targetActor,  targetPackage)

// Count how many package overrides are currently on this actor. It will also count ones that's condition isn't met.
export const CountPackageOverride = (targetActor: Actor | null | undefined): number => sn.CountPackageOverride(targetActor)

// Remove all package overrides on this actor, including ones that were added by other mods.
export const ClearPackageOverride = (targetActor: Actor | null | undefined): number => sn.ClearPackageOverride(targetActor)

// Remove this package from all actor overrides.
export const RemoveAllPackageOverride = (targetPackage: Package | null | undefined): number => sn.RemoveAllPackageOverride(targetPackage)
